...
namespace Vdc.Pos.Api.Controllers
{
    ...
    public class VariationsController : ControllerBase
    {
        ...
        [HttpPost("CreateMultiVariation")]
        public async Task<ActionResult<IEnumerable<VariationResponseDto>>> CreateMultiVariationAsync(MultiVariationRequestDto multiVariationRequestDto)
        {
            try
            {
                var variationsCreated = await _variationService.InsertMultipleVariationAsync(multiVariationRequestDto);
                return Ok(variationsCreated);
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
